...
function getFirebaseError(code: string): string {
  switch (code) {
    case "auth/email-already-in-use":   return "This email is already in use. Sign in instead?";
      ...
      >
        <View className="flex-1 px-6 pt-14 pb-10">
          ...
          <View className="items-center mb-8">
            ...
            <Text className="text-[13px] text-[#7C7CB0] mt-1 tracking-wide">
              Create your account
            </Text>
          </View>
          ...
          <View className="gap-4">
            ...
            <View className="gap-[6px]">
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  <TextInput
                    className={`${inputBase} ${touchedFields.name && errors.name ? borderErr : borderOk}`}
                    placeholder="Kiran Mayi"
              ...
              {touchedFields.name && errors.name && (
                <View className="flex-row items-center gap-1 ml-1">
                  ...
                  <Text className="text-red-400 text-[12px]">{errors.name.message}</Text>
                </View>
              )}
            </View>
            ...
            <View className="gap-[6px]">
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  <View className={`flex-row bg-white rounded-2xl px-4 h-[52px] items-center ${touchedFields.confirmPassword && errors.confirmPassword ? borderErr : borderOk}`}>
                    <TextInput
                      ref={confirmRef}
                      className="flex-1 text-[#1E1B4B] text-[15px]"
                    ...
                    <TouchableOpacity onPress={() => setShowConfirm((p) => !p)} className="pl-2">
                      <Ionicons name={showConfirm ? "eye-off-outline" : "eye-outline"} size={20} color="#7C7CB0" />
                    </TouchableOpacity>
                  </View>
                )}
              />
              {touchedFields.confirmPassword && errors.confirmPassword && (
                <View className="flex-row items-center gap-1 ml-1">
                  <Ionicons name="alert-circle-outline" size={13} color="#f87171" />
                  <Text className="text-red-400 text-[12px]">{errors.confirmPassword.message}</Text>
                </View>
              )}
            </View>
            ...
            <TouchableOpacity
              className="bg-[#7C5FFF] rounded-2xl h-[54px] items-center justify-center mt-2"
              style={{
                shadowColor: "#7C5FFF", shadowOffset: { width: 0, height: 8 },
            ...
            >
              {loading ? (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  ...
                  <Text className="text-white font-bold text-[16px]">Creating account…</Text>
                </View>
              ) : (
                <Text className="text-white font-bold text-[16px] tracking-widest">
                  Create Account
                </Text>
              )}
            </TouchableOpacity>
            ...
            >
              ...
              <Text className="text-[#1E1B4B] font-semibold text-[15px]">
                Sign up with Google
              </Text>
            </TouchableOpacity>
            ...
            <View className="flex-row justify-center items-center gap-1 mt-1">
              <Text className="text-[14px] text-[#7C7CB0]">Already have an account?</Text>
              <TouchableOpacity activeOpacity={0.7} onPress={() => router.back()}>
                <Text className="text-[14px] text-[#7C5FFF] font-semibold"> Sign in</Text>
              </TouchableOpacity>
            </View>
